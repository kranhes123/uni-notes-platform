  ...
  try {
    ...
    if (email.isEmpty || password.isEmpty) {
      return Response.json(
        statusCode: 400,
        body: {'message': isEnglish ? 'Email and password are required.' : 'E-posta ve şifre gerekli.'},
      );
    }
    ...
    if (user == null) {
      return Response.json(
        statusCode: 401,
        body: {'message': isEnglish ? 'User not found.' : 'Kullanıcı bulunamadı.'},
      );
    }

    // Mail doğrulanmamışsa girişi engelle
    ...
    final storedPasswordHash = user['passwordHash']?.toString() ?? '';
    if (storedPasswordHash != hashPassword(password)) {
      return Response.json(
        statusCode: 401,
        body: {'message': isEnglish ? 'Incorrect password.' : 'Şifre yanlış.'},
      );
    }
    ...
    return Response.json(
      statusCode: 200,
      body: {
        'message': isEnglish ? 'Login successful' : 'Giriş başarılı',
        'user': {
    ...
    return Response.json(
      statusCode: 500,
      body: {'message': isEnglish ? 'An error occurred' : 'Hata oluştu', 'error': e.toString()},
    );
  }
}